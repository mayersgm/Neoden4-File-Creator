...
class N4SortMenu(ttk.Frame):
    ...
    def create_widgets1(self, frame1, frame2):
        ...
        buttons = [
            ...
            (" Split PCR   ", 6,0, self.process_pcb,'green'),
            ("Generate CSV",  6,1, self.generate_csv,'red'),
            (" Help Guide  ", 6,2, self.show_help, 'blue')
        ]
        
        for text, row, column, command, fg in buttons:
            if command:  # Button
                btn = tk.Button(
                    frame1 if row < 4 else frame2,
                    text=text,
                    command=command,
                    fg = fg,
            ...
            if row >= 6:
                btn.grid(row=row, column=column, padx=5,  pady=10) #if column <2 else 0,
            else:
                btn.grid(row=row, column=0, padx=6 if row < 4 else 1, 
                        pady=5 if row < 4 else 10)
        
         # Add help button wi
         # th some visual distinction
        help_btn = tk.Button(
            frame2,
            text="Help Guide",
            command=self.show_help,
            fg='blue',  # Make help button stand out
            font=("Times New Roman", 11, "bold")
        )
       # help_btn.grid(row=6, column=3, padx=6, pady=5)
    ...
    def create_radio_button2(self):
        """Create sorting options with order selection"""
        # Define headers
        tk.Label(self.frame2, text="Sorting Configuration", 
                font=("Times New Roman", 12, BOLD)).grid(row=1, column=0, columnspan=4, pady=4)

        # Create frame for sorting options
        sort_frame = ttk.LabelFrame(self.frame2, text="Available Sort Options")
        sort_frame.grid(row=2, column=0, columnspan=2, padx=10, pady=5, sticky="nsew")

        # Create frame for selected options
        selected_frame = ttk.LabelFrame(self.frame2, text="Selected Sort Order")
        selected_frame.grid(row=2, column=2, columnspan=2, padx=10, pady=5, sticky="nsew")
        ...
        for key, value in self.sort_options.items():
            self.available_listbox.insert(tk.END, value)
        self.available_listbox.grid(row=0, column=0, padx=5, pady=5)

        # Create Listbox for selected options
        self.selected_listbox = tk.Listbox(selected_frame, height=6, width=25)
        self.selected_listbox.grid(row=0, column=0, padx=5, pady=5)
        ...
        options_frame = ttk.LabelFrame(self.frame2, text="Sort Options")
        options_frame.grid(row=4, column=0,rowspan=2, columnspan=4, padx=10, pady=5, sticky="nsew")

        # Add sort order option
        self.sort_order_var = tk.BooleanVar(value=True)
        ttk.Radiobutton(options_frame, text="Ascending", variable=self.sort_order_var,
                        value=True).grid(row=0, column=0, padx=5)
        ttk.Radiobutton(options_frame, text="Descending", variable=self.sort_order_var,
                        value=False).grid(row=0, column=1, padx=5)

        # Add sort inplace option
        self.sort_inplace_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(options_frame, text="Sort Inplace",
                        variable=self.sort_inplace_var).grid(row=0, column=2, padx=10)

        # Add PCB side selection
        self.pcb_side_var = tk.StringVar(value="False")
        ttk.Radiobutton(options_frame, text="Top", variable=self.pcb_side_var,
                        value="False").grid(row=0, column=3, padx=5)
        ttk.Radiobutton(options_frame, text="Bottom", variable=self.pcb_side_var,
                        value="True").grid(row=0, column=4, padx=5)
...
class PCBDataProcessor:
    ...
    def generate_csv1(self, pcb_file: str, template_file: str, pcb_width: float,
                    sort_config: Dict, 
        ...
        try:
            ...
            # If no PCB width provided, calculate from fiducials
            if pcb_width <= 0:
            ...
            if progress_callback:
                progress_callback(2, 5, f"Using PCB width: {pcb_width}")
            ...
            # Get fiducial information and process components
            processed_df = self.process_components(
                pcb_df=pcb_df,
                n4_df=n4_df,
                sort_config=sort_config,
                pcr_path=os.path.dirname(pcb_file)  # Pass directory path for saving unmatched components
            )
    ...
    def process_components(self, pcb_df: pd.DataFrame, n4_df: pd.DataFrame,
        ...
        try:
            ...
            if 'XY_DIST' in sort_config.get('columns', []):
                # Get fiducial information for distance calculation
            ...
            if 'XY_DIST' in pcb_df.columns and 'XY_DIST' not in sort_config.get('columns', []):
                pcb_df.drop(columns=['XY_DIST'], inplace=True)

            # Process components through nozzle assignment
            processed_df = self.nozzle_feeder_assignment(pcr_path, pcb_df, n4_df)
            return processed_df
    ...
    def get_pcb_width_from_fiducials1(self, pcb_df: pd.DataFrame) -> float:
        """
        Calculate PCB width from fiducial positions
        
        Args:
            pcb_df: DataFrame containing PCB component data
            
        Returns:
            float: Calculated PCB width from rightmost fiducial
        """
        try:
            ...
            # Sort by X descending and Y ascending to get rightmost fiducial with lowest Y
            sorted_fiducials = fiducials.sort_values(['SYM_X', 'SYM_Y'], ascending=[False, True])
            
            # Get the X value of the first fiducial (largest X, smallest Y)
            pcb_width = float(sorted_fiducials.iloc[0]['SYM_X'])
            
            self.logger.info(f"Calculated PCB width from fiducials: {pcb_width}")
            return pcb_width
    ...
    def generate_csv(self, pcb_file: str, template_file: str, pcb_width: float,
        ...
        try:
            ...
            if pcb_width <= 0:
                ...
                # Verify the calculated width
                self.verify_pcb_width(pcb_width, pcb_df)
    ...
    def process_components2(self, pcb_df: pd.DataFrame, n4_df: pd.DataFrame,
                         fiducial_info: Dict, sort_config: Dict,
                         pcb_file: str) -> pd.DataFrame:
        pcb_df = pcb_df[
        ...
        try:
            ...
            # Process remaining component data
            ...
            pcb_df = self.nozzle_feeder_assignment(
                os.path.dirname(pcb_file), pcb_df, n4_df
            )
            ...
            logging.error(f"Error processing components: {str(e)}")
            raise PCBProcessingError(f"Failed to process components: {str(e)}")

    def get_fiducial_info(self, n4_df: pd.DataFrame, 
                         pcb_df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            # Get reference point
            n4_offsetx = float(measured_fiducial_loc[col_names[3]])
            n4_offsety = float(measured_fiducial_loc[col_names[4]])

            # Get fiducial points from PCB and template
            fiducial_pts_i = self.get_board_fiducials(pcb_df)
            fiducial_pts_m = [[float(measured_fiducial_loc[col_names[i*2+3]]),
                              float(measured_fiducial_loc[col_names[i*2+4]])] 
                             for i in range(len(fiducial_pts_i))]
            
            # 3. Sort by distance from origin (0,0)
            fiducial_pts_mr = sorted(fiducial_pts_m, key=lambda point: math.sqrt(point[0]**2 + point[1]**2))
            print("Sorted by distance from origin:", fiducial_pts_m, fiducial_pts_mr)
            
            #Get the first xy location
            for i in range(1):
                n4_offsetx, n4_offsety = fiducial_pts_i[i]
            ...
            logging.error(f"Error in get_fiducial_info: {str(e)}")
    ...
    def get_board_fiducials(self, pcb_df: pd.DataFrame) -> List[List[float]]:
        ...
        try:
            fid_df = pcb_df[pcb_df['REFDES'].str.contains('FID')]
            ...
            return fid_df[['SYM_X', 'SYM_Y']].values.tolist()

        except Exception as e:
            logging.error(f"Error in get_board_fiducials: {str(e)}")
...
class PCR_File_Splitter:
    ...
    def _merge_component_group_data2(self, template: pd.DataFrame, row_index: int, group: Dict):
        """Update template row with component data"""
        reel_size = str(group['Reel'])
        feeder_id = template.at[row_index, 'Feeder ID']
        
        # If it's feeder 20, force nozzle to 1
        nozzle = '1' if feeder_id == '20' else template.at[row_index, 'Nozzle']
        
        # Update the template row
        template.at[row_index, 'Footprint'] = group['footprint']
        template.at[row_index, 'Value'] = f"{group['footprint']}/{group['value']}"
        template.at[row_index, 'Nozzle'] = nozzle
        
        # Use original template values for pick/place parameters
        template.at[row_index, 'Pick height'] = self.component_table_df[
            self.component_table_df['Footprint'] == group['footprint']
        ]['Pick height'].iloc[0]
        
        template.at[row_index, 'Place Height'] = self.component_table_df[
            self.component_table_df['Footprint'] == group['footprint']
        ]['Place Height'].iloc[0]
        
        self.logger.info(
            f"Updated template row {row_index} with component "
            f"{group['footprint']}/{group['value']} "
            f"(Count: {group['count']}, Reel: {reel_size}, Feeder: {feeder_id}, Nozzle: {nozzle})"
        )

    def _process_group2(self, pcr_data: pd.DataFrame, group_name: str):
        """Process a group of PCR data with smart feeder assignment - high count groups first"""
        try:
            # Initialize dataframes
            manual_placement = pd.DataFrame(columns=pcr_data.columns)
            fiducials = pd.DataFrame(columns=pcr_data.columns)
            template_a = self.neoden4_df.copy()
            template_b = self.neoden4_df.copy()
            pcr_a = pd.DataFrame(columns=pcr_data.columns)
            pcr_b = pd.DataFrame(columns=pcr_data.columns)

            # Step 1: Create component groups and sort by count (20% of progress)
            if self.progress_callback:
            ...
            # Sort high count groups by count descending (most placements first)
            # Sort low count groups by count descending (to optimize remaining feeder usage)
            ...
            # Process components in sorted order (60% of progress)
            total_groups = len(low_count_groups) + len(high_count_groups)
            current_group = 0

            # First process all high count groups (must skip feeder #20)
            ...
            for group in high_count_groups:
                ...
                # Try placement in templates without feeder 20
                ...
                if result == PlacementResult.NOT_PLACED:
                    manual_placement = pd.concat([manual_placement, pd.DataFrame(c['pcr_row'].to_dict() 
                        for c in group['components'])], ignore_index=True)
            ...
            # Then process low count groups (can use any remaining feeders, including #20)
            self.logger.info("Processing low count groups with remaining feeders...")
            for group in low_count_groups:
                ...
                if self._is_fiducial(group['components'][0]['pcr_row']):
                    fiducials = pd.concat([fiducials, pd.DataFrame(c['pcr_row'].to_dict() 
                        for c in group['components'])], ignore_index=True)
                    current_group += 1
                    self.logger.info(f"Found fiducials stage2 {fiducials}")
                    continue
                # Try placement in templates with any available feeder
                #placed = False
                for template, pcr_df in [(template_a, pcr_a), (template_b, pcr_b)]:
                    self._reset_available_feeders()
                ...
                # If placement failed, add to manual placement        
                if result == PlacementResult.NOT_PLACED:
                    manual_placement = pd.concat([manual_placement, pd.DataFrame(c['pcr_row'].to_dict() 
                        for c in group['components'])], ignore_index=True)
                    self.unmatched_count += group['count']
            ...
            # Step 3: Add fiducials and prepare final files (10% of progress)
            if self.progress_callback:
                self.progress_callback(80, 100, f"Adding fiducials to PCR files for {group_name}")
                
            # Add fiducials to both PCR files
            pcr_a = pd.concat([pcr_a, fiducials], ignore_index=True)
            pcr_b = pd.concat([pcr_b, fiducials], ignore_index=True)
           # self.logger.info(f"Found fiducials {fiducials}")

            # Step 4: Save files (10% of progress)
            if self.progress_callback:
                self.progress_callback(90, 100, f"Saving template and PCR files for {group_name}")
...
if __name__ == "__main__":
    app = N4SortGUIApp("Neoden4 CSV Creator v2i", (545, 600))
