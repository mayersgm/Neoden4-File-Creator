...
class PCBDataProcessor:
    ...
    def nozzle_feeder_assignment2(self, pcr_path: str, pcb_df: pd.DataFrame, n4_df: pd.DataFrame) -> pd.DataFrame:
        """
        Assign nozzles and feeders to components based on rotation capabilities
        """
        try:
            ...
            for _, row in n4_df.iterrows():
                ...
                # Preserve original nozzle format
                nozzle_str = str(row['Nozzle'])
                if len(nozzle_str) == 1:
                    available_nozzles = [nozzle_str]
                else:
                ...
                if value not in n4_map:
                    n4_map[value] = {
                        '#Feeder': feeder_id,
                        'Available_Nozzles': available_nozzles,
                        'Original_Nozzle': nozzle_str
                    }
            ...
            # First count components per feeder
            for _, row in pcb_df.iterrows():
                comp_value = str(row['COMP_VALUE']).strip()
                sym_name = str(row['SYM_NAME']).strip()
                combined_value = f"{sym_name}/{comp_value}"
            ...
            for index, row in pcb_df.iterrows():
                ...
                if combined_value in n4_map:
                    ...
                    if valid_nozzles:
                        nozzle_counts = {n: feeder_nozzle_assignment[feeder_id].count(n) for n in valid_nozzles}
                        chosen_nozzle = min(nozzle_counts.keys(), key=lambda k: nozzle_counts[k])
                        
                        pcb_df.at[index, '#Feeder'] = 'comp'
                        pcb_df.at[index, 'Feeder ID'] = feeder_id
                        pcb_df.at[index, 'Type'] = '0'
                        pcb_df.at[index, 'Nozzle'] = n4_map[combined_value]['Original_Nozzle']
    ...
    def generate_output2(self, processed_df: pd.DataFrame, template_df: pd.DataFrame, input_file: str) -> str:
        """
        Generate final output CSV file with proper index handling.
        """
        try:
            # Ensure all columns exist in both DataFrames with proper defaults
            all_columns = ['#Feeder', 'Feeder ID', 'Type', 'Nozzle', 'Name', 'Value', 
                        'Footprint', 'X', 'Y', 'Rotation', 'Skip', 'Pick height',
                        'Pick delay', 'Place Height', 'Place Delay', 'Vacuum detection',
                        'Threshold', 'Vision Alignment', 'Speed']
            
            # Reset indices and drop any old index columns
            template_df = template_df.reset_index(drop=True)
            processed_df = processed_df.reset_index(drop=True)
            
            # Fill missing columns in processed_df
            for col in all_columns:
                if col not in processed_df.columns:
                    if col in ['Pick height', 'Place Height']:
                        processed_df[col] = 2.2
                    elif col in ['Pick delay', 'Place Delay']:
                        processed_df[col] = 100
                    elif col == 'Vacuum detection':
                        processed_df[col] = 'No'
                    elif col == 'Threshold':
                        processed_df[col] = -40
                    elif col == 'Vision Alignment':
                        processed_df[col] = 1
                    elif col == 'Speed':
                        processed_df[col] = 80
                    else:
            ...
            final_df = pd.concat([template_df, processed_df], ignore_index=True)
            ...
            self.logger.error(f"Error in generate_output: {str(e)}")
            raise PCBProcessingError(f"Failed to generate output file: {str(e)}")
